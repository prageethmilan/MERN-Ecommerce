...
export const resizeProductImages = asyncHandler(async (req, res, next) => {
  ...
  if (req.files.image) {
    ...
    await sharp(req.files.image[0].buffer)
      ...
      .jpeg({quality: 90})
      .toFile(
        `H:\\Github Projects\\MERN-Ecommerce\\${process.env.FILES_UPLOADS_PATH}\\products\\${mainImageFilename}`
      );
    ...
    await Promise.all(
      req.files.sliderImages.map(async (img, idx) => {
        ...
        await sharp(img.buffer)
          ...
          .jpeg({quality: 90})
          .toFile(
            `H:\\Github Projects\\MERN-Ecommerce\\${process.env.FILES_UPLOADS_PATH}\\products\\${sliderImageName}`
          );
